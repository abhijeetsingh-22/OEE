...
function QuizResultTable() {
	...
	useEffect(() => {
		const fetch = async () => {
			try {
				const submissionData = await apiCall(
					'get',
					`/api/evaluation/${evaluationId}/submissions?type=quiz`
				)
	...
	const tableData = submissions.map((userSub) => {
		...
		var userScores = evaluation.map((question) => {
			var attempt = userSub.questions.find((sub) => {
				console.log('table data user', sub, question)
				console.log('helloj q', sub)
				return sub.question === question._id
			})
